...
const AdminDashboard = () => {
  ...
  const res = await fetch(`${API_BASE_URL}/api/hr/accounts`);
    const data = await res.json();
    setHrAccounts(data);
  };

  const fetchSignupRequests = async () => {
  const empRes = await fetch(`${API_BASE_URL}/api/employees/pending`);
    const empData = await empRes.json();

  const hrRes = await fetch(`${API_BASE_URL}/api/hr/pending`);
  ...
  await fetch(`${API_BASE_URL}/api/employees/approve/${data.id}`, { method: "POST" });
      fetchEmployees();
      fetchSignupRequests();
  ...
  const handleAddHr = async (e) => {
    e.preventDefault();
  const response = await fetch(`${API_BASE_URL}/api/hr/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (response.ok) {
      const data = await response.json();
  await fetch(`${API_BASE_URL}/api/hr/approve/${data.id}`, { method: "POST" });
      fetchHrAccounts();
      fetchSignupRequests();
      setShowAddHr(false);
      setNewHr({
  ...
  const handleUpdateEmployee = async (e) => {
    e.preventDefault();
  await fetch(`${API_BASE_URL}/api/employees/${editEmployee.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  const handleUpdateHr = async (e) => {
    e.preventDefault();
  await fetch(`${API_BASE_URL}/api/hr/${editHr.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  await fetch(`${API_BASE_URL}/api/hr/${id}`, { method: "DELETE" });
    fetchHrAccounts();
  };
  ...
  url = `${API_BASE_URL}/api/employees/approve/${request.id}`;
    } else {
  url = `${API_BASE_URL}/api/hr/approve/${request.id}`;
    }
  ...
  const handleRejectRequest = async (request) => {
    let url = "";
    if (request.role === "employee") {
  url = `${API_BASE_URL}/api/employees/reject/${request.id}`;
    } else {
  url = `${API_BASE_URL}/api/hr/reject/${request.id}`;
    }
